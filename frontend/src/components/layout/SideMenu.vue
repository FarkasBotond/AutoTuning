...
const categories = computed(() => serviceCategoryStore.categories)
...
onMounted(async () => {
    if (serviceCategoryStore.categories.length === 0) {
        await serviceCategoryStore.fetchAllCategories()
    }
})
</script>
        ...
        <div class="p-3">
            <div v-if="activeTab === 'categories'" class="space-y-3">
                <div v-for="item in categories" :key="item.id"
                    class="group relative flex h-[72px] cursor-pointer items-center overflow-hidden rounded-xl border px-4 transition duration-200 hover:-translate-y-0.5 hover:border-teal-200 hover:bg-teal-50"
                    :class="selectedCategoryId === item.id
                        ? 'border-teal-400 bg-teal-50'
                        : 'border-zinc-200 bg-white'" @click="selectCategory(item.id)">
            ...
            <div v-else class="grid grid-cols-1 gap-2">
                <button v-for="brand in brands" :key="brand" type="button"
