<template>
  <div class="flex min-h-screen items-center justify-center px-4 py-10 md:px-6">
    <div class="glass-panel grid w-full max-w-5xl overflow-hidden lg:grid-cols-[1.1fr_1fr]">
      <aside
        class="hidden bg-gradient-to-br from-teal-700 via-teal-800 to-zinc-900 p-10 text-white lg:flex lg:flex-col lg:justify-between">
        <div>
      ...
      <section class="p-6 sm:p-8 lg:p-10">
        ...
        <form @submit.prevent="handleLogin" class="mt-8 flex flex-col gap-5">
          ...
          <div class="flex flex-col gap-2">
            ...
            <input id="email" v-model="formData.email" type="email" placeholder="pelda@email.com" required
              :disabled="authStore.isLoading" @focus="authStore.clearError()" class="brand-input" />
          </div>
        ...
        <div class="mt-6 text-center text-sm text-zinc-600">
          ...
          <router-link to="/register"
            class="font-semibold text-teal-700 transition-colors hover:text-teal-800">Regisztrálj itt</router-link>
        </div>
      </section>
    </div>
  </div>
